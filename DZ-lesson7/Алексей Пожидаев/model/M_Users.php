...
{	
	...
	{		
		...
		$this -> result1 = $this -> priv_query_help ('users', $this -> where);
		$this -> role_ =($this -> result1[0]);
		$role = (int) $this -> role_ ['id_role'];
		$this -> where = "id_role = '$role'";
		$this -> result2 = $this -> priv_query_help ('privs2roles', $this -> where);
				
		foreach ($this -> result2 as $roles)
		{
			$id_role = (int) $roles['id_priv'];
			$this -> where = "id_priv = '$id_role'";
			$this -> result3 = $this -> priv_query_help ('privs', $this -> where);
				foreach ($this -> result3 as $role)
				{
					if ($role['name'] == $priv)
					{
						return true;
	...
	//
	public function IsOnline($id_online)
	{		
		$this -> qstring = "SELECT `id_user` FROM `sessions` WHERE `id_user` = '%d'";
		$query = sprintf($this -> qstring, mysql_real_escape_string($id_online));
		$this -> result = $this->msql->Select($query);
	...
	public function Users_online() 
	{
		...
		$this -> result = $this->msql->Select($this -> query);
		$this -> online = array();
		 			
			foreach ($this -> result as $userarray)
			{
				$this -> IdUser = (int) ($userarray ['id_user']);
				if ($this -> IsOnline ($this -> IdUser)) 
				{
					$this -> online [] = $userarray ['name'];
				}
			}
			return ($this -> online);
	}
	private function priv_query_help ($table, $where)
	{
		$this -> where = (int) $where;
		$this -> qstring = "SELECT * FROM `%s` WHERE $where ";
		$query = sprintf($this -> qstring, $table, $this -> where);
		$this -> result = $this->msql->Select($query);
		return $this -> result;
	}
