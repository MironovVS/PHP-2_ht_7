...
{	
	...
	public function ChangeUserRole($id_user,$id_role){
		...
		if($this->msql->Update('users', $user, $where)){
			return true;
		}
		else return false;
	}
	...
	public function AddRole($name,$description){
		...
		$this->msql->Insert('roles', $data); 
	}
	public function DelRole($role_id){
		$where=sprintf('id_role=\'%s\'',$role_id);
		$this->msql->Delete('roles',$where);
		$this->msql->Delete('privs2roles',$where);
	}
	...
	public function AddP2R($role,$priv){
		...
		$check_role=$this->msql->Select($check_role_sql);
		$check_priv=$this->msql->Select($check_priv_sql);
		if(!empty($check_role) && !empty($check_priv)){
			...
			$check_p2r=$this->msql->Select($check_p2r_sql);
			if(empty($check_p2r)){
				$data['id_role']=$role;
				$data['id_priv']=$priv;
				$this->msql->Insert('privs2roles',$data);
			}
		}
	}
